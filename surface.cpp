...
Surface Surface::GetPartialCopy(int src_start_X, int src_start_Y, int width, int height)
{
	Surface partial_copy(width, height);
	partial_copy.CopyFrom(this, src_start_X, src_start_Y, width, height);
	return partial_copy;
}

// Copy a defined area from the source to the destination.
void Surface::CopyFrom(Surface* a_Src, int src_start_X, int src_start_Y, int width, int height)
{
	Pixel* dst = m_Buffer;
	Pixel* src = a_Src->GetBuffer();;
	if ((src) && (dst))
	{
		int min_height = (m_Height < a_Src->GetHeight() ? m_Height : a_Src->GetHeight());
		int min_width = (m_Width < a_Src->GetWidth() ? m_Width : a_Src->GetWidth());
		
		src += src_start_X + (src_start_Y * m_Pitch);

		for (int y{ src_start_Y }; y <= height; y++)
		{
			if (y < 0 || y >= min_height) continue;

			for (int x{ 0 }; x <= width; x++)
			{
				if (x + src_start_X < 0 || x + src_start_X > min_width) continue;
...
Sprite::Sprite(Surface* a_Surface, unsigned int a_NumFrames, bool isOwnerOfSurface) :
	...
	m_Surface(a_Surface),
	m_isOwnerOfSurface(isOwnerOfSurface)
{
	InitializeStartData();
}
